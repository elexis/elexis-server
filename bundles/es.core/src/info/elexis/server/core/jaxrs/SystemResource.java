...
public class SystemResource {
	...
	public Response getLogFileOfToday(){
		File logFile =
			CoreUtil.getElexisServerHomeDirectory().resolve("logs/elexis-server.log").toFile();
		if (logFile.exists() && logFile.canRead()) {
			ResponseBuilder response = Response.ok(logFile);
			response.header("Content-Disposition",
				"attachment;filename=" + logFile.getName() + ".txt");
	...
	@ApiResponses(value = {
		@ApiResponse(code = 422, message = "restart was vetoed, see message for reason")
	})
