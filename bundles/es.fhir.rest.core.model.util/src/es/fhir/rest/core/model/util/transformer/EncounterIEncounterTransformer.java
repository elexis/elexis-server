...
public class EncounterIEncounterTransformer implements IFhirTransformer<Encounter, IEncounter> {
	...
	public Optional<IEncounter> createLocalObject(Encounter fhirObject){
		...
		Optional<IMandator> performerKontakt =
			codeModelService.load(encounterHelper.getMandatorId(fhirObject).get(), IMandator.class);
		Optional<IPatient> patientKontakt =
			codeModelService.load(encounterHelper.getPatientId(fhirObject).get(), IPatient.class);
