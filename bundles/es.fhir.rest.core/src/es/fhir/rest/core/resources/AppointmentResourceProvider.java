...
public class AppointmentResourceProvider implements IFhirResourceProvider {
	...
	@Read
	public Appointment getResourceById(@IdParam
	IdType theId){
		String idPart = theId.getIdPart();
		if (idPart != null) {
			Optional<IAppointment> appointment = modelService.load(idPart, IAppointment.class);
			if (appointment.isPresent()) {
				Optional<Appointment> fhirAppointment =
					getTransformer().getFhirObject(appointment.get());
	...
	public List<Appointment> findAppointmentsByDateAndOptionallyArea(
		@RequiredParam(name = Appointment.SP_DATE)
		DateRangeParam dateParam,
		@Description(shortDefinition = "Only supports Schedule/identifier")
		@OptionalParam(name = Appointment.SP_ACTOR)
		ReferenceOrListParam actors, @IncludeParam(allow = {
			"Appointment:patient"
		})
	...
	@Update
	public MethodOutcome updateAppointment(@IdParam
