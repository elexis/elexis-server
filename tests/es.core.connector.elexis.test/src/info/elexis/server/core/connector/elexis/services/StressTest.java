...
public class StressTest {
	...
	public void stressTest() throws InterruptedException {
		...
		Kontakt mandator = KontaktService.INSTANCE.createPatient("Mandator", "Mandator", LocalDate.now(), Gender.MALE);

		for (int i = 0; i < upperBound; i++) {
			Kontakt patient = KontaktService.INSTANCE.createPatient("FirstName" + i, "LastName" + i, LocalDate.now(),
					Gender.FEMALE);
			patients.add(patient.getId());

			KontaktService.INSTANCE.flush();

			Fall fall = FallService.INSTANCE.create(patient, "label" + i, "reason" + i, "billingMethod" + i);
			faelle.add(fall.getId());

			FallService.INSTANCE.flush();

			Behandlung behandlung = BehandlungService.INSTANCE.create(fall, mandator);
...
//		t2.join();
		...
		for (String string : consultations) {
			Optional<Behandlung> findById = BehandlungService.INSTANCE.findById(string);
