...
public class StockServiceTest extends AbstractServiceTest {
	...
	private static StockService stockService = StockService.INSTANCE;
	...
	public static void init() {
		...
		article_A = ArtikelstammItemService.INSTANCE.findById(TestEntities.ARTIKELSTAMM_ITEM_PHARMA_ID).get();
		...
		item3 = ArtikelstammItemService.INSTANCE.findById(TestEntities.ARTIKELSTAMM_ITEM_PHARMA_ID_3).get();
		artikel_item3 = StoreToStringService.storeToString(item3);

		defaultStock = StockService.INSTANCE.findById("STD").get();
	...
	public void testPerformDisposalAndReturnOfArticle() {
		...
		Stock stock = StockService.INSTANCE.create("TMP", 20);
		IStockEntry se = stockService.storeArticleInStock(stock,
		...
		StockService.INSTANCE.remove(stock);
	}
	...
	public void testGetCummulatedCountForArticle() {
		...
		StockEntryService.INSTANCE.write((StockEntry) stockEntry_C);
		...
		StockEntryService.INSTANCE.write((StockEntry) stockEntry_D);
		...
		StockEntryService.INSTANCE.write((StockEntry) stockEntry_Dd);

		Integer cumulatedStockForArticle = StockService.INSTANCE.getCumulatedStockForArticle(article_A);
		assertEquals(20, cumulatedStockForArticle.intValue());

		Availability availability = StockService.INSTANCE.getCumulatedAvailabilityForArticle(article_A);
		assertEquals(Availability.IN_STOCK, availability);

		Availability availability3 = StockService.INSTANCE.getCumulatedAvailabilityForArticle(item2);
		assertEquals(Availability.CRITICAL_STOCK, availability3);

		Availability availability4 = StockService.INSTANCE.getCumulatedAvailabilityForArticle(item2);
